...
import { listOrdersByEmail } from "@/features/order/order.service";
...
export default async function OrderHistoryPage() {
  ...
  const orders: OrderRecord[] = listOrdersByEmail(user.email);
        ...
        <div className="mx-auto max-w-page px-4 py-8 sm:px-6 lg:px-8">
          ...
          {orders.length > 0 ? (
            <div className="mt-8 flex flex-col gap-4">
              {orders.map((order) => (
                <div key={order.code} className="overflow-hidden rounded-panel border border-slate-200 bg-white">
                  <div className="flex items-center justify-between border-b border-slate-100 px-6 py-4">
                    <div>
                      <p className="font-display text-sm font-bold text-midnight">{order.code}</p>
                      <p className="mt-0.5 text-xs text-slate-500">{order.items.length} sản phẩm</p>
                    </div>
                    <div className="flex items-center gap-3">
                      <span className={`inline-flex items-center rounded-full px-3 py-1 text-xs font-bold ${order.status === "PAID" ? "bg-teal-50 text-teal-tech" : order.status === "FAILED" || order.status === "CANCELLED" ? "bg-red-50 text-red-600" : "bg-amber-50 text-amber-600"}`}>
                        {order.status === "PAID" ? "Đã thanh toán" : order.status === "FAILED" ? "Thất bại" : order.status === "CANCELLED" ? "Đã hủy" : "Chờ xác nhận"}
                      </span>
                      <span className="font-display text-lg font-bold text-electric">{formatVnd(order.grandTotal)}</span>
                    </div>
                  </div>
                  ...
                  <div className="border-t border-slate-100 px-6 py-4">
                    <Link
                      href={`/don-hang/${order.code}?token=${order.trackingToken}`}
